...
describe('ReleaseChart', () => {
  it('plots only supplied scores and exposes the exact data in an accessible table', () => {
    ...
    const chart = screen.getByRole('img', { name: 'Release scores compared with file size' })
    ...
    expect(Number(points[0].getAttribute('cy'))).toBeGreaterThan(Number(points[1].getAttribute('cy')))
    const table = screen.getByRole('table', { name: 'Release score data' })
    expect(within(table).getAllByRole('columnheader').map((cell) => cell.textContent)).toEqual(['Release', 'Release size (GiB)', 'Policy score'])
    expect(within(table).getAllByRole('row').slice(1).map((row) => row.textContent)).toEqual(['small2.5-12', 'large18.4145.25'])
  ...
  it('clears stale points and explains when no valid data remains', () => {
    const { rerender } = render(<ReleaseChart rows={[{ id: 'old', size: 1, score: 2 }]} />)
    rerender(<ReleaseChart rows={[]} />)
    expect(screen.getByText('No valid release scores available.')).toBeInTheDocument()
